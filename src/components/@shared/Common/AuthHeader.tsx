...
export default function AuthHeader() {
  ...
  const isMyPages = pathname.includes('mydashboard') || pathname.includes('mypage');
  ...
  return (
    <Flex className="fixed left-0 right-0 top-0 z-50 h-[60px] items-center justify-end gap-3 border-b-[1px] border-border-gray bg-white pl-[84px] pr-3 md:h-[70px] md:gap-6 md:px-10 md:pl-[200px] md:pr-10 xl:gap-8 xl:px-[70px] xl:pl-[340px] xl:pr-20">
      <div className={`grow items-center font-xl-20px-bold xl:flex xl:gap-2 ${isMyPages ? 'flex' : 'hidden'}`}>
        <h1>{titleValue}</h1>
        {createdByMe && <Image width={20} height={16} src="/icons/crown.png" alt="createByMe" />}
      </div>
      <Group className="gap-[6px] font-md-14px-medium">
        <HeaderButton>
          <Image className="hidden md:mr-2 md:inline" width={20} height={20} src="/icons/settings.png" alt="settings" />
          <Link href="/mypage">관리</Link>
        </HeaderButton>
        <HeaderButton>
          <Image className="hidden md:mr-2 md:inline" width={20} height={20} src="/icons/add_box.png" alt="settings" />
          초대하기
        </HeaderButton>
      </Group>
      ...
      {user && (
        <Flex className="items-center gap-3">
          <Avatar>{user.profileImageUrl && <Image src={user.profileImageUrl} alt="my profile" />}</Avatar>
          <Text className="hidden font-lg-16px-medium md:block">{user.nickname}</Text>
        </Flex>
      )}
    </Flex>
  );
