...
export default function ColumnList() {
  const { columnList, dashboardColor, onSubmitCreateColumn, handleSubmit, register, errors } = useColumns();
  ...
  return (
    <>
      <div className="no-scrollbar flex min-h-[100vh] flex-col md:mx-0 md:max-w-full xl:max-w-[1062px] xl:flex-row xl:overflow-scroll">
        {columnList?.length > 0 &&
          columnList.map(column => <Column key={column.id} column={column} dashboardColor={dashboardColor} />)}
      </div>
